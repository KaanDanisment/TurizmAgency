...
import PatikaTurizmAgency.Model.Price;
...
public class CreateReservationGUI extends JFrame {
    ...
    public CreateReservationGUI(Room room, Hotel hotel, int roomCount, int adultCount, int childCount, int days, Date firstDate, Date secondDate) {
        this.hotel = hotel;
        this.room = room;
        ...
        String entranceDate = dateFormat.format(firstDate);
        String releaseDate = dateFormat.format(secondDate);
        ...
        bttn_calculate.addActionListener(e -> {
            int price = Price.calculate(hotel, room.getRoomType(), days, cmb_pension_type.getSelectedItem().toString(), roomCount, adultCount, childCount);
